...
func (r *PaymentRepository) GetByIdempotencyKey(ctx context.Context, key string) (*model.Payment, error) {
	query := `
        SELECT id, order_id, user_id, store_id, amount, currency, status, payment_method,
               ...
               idempotency_key, failure_reason, webhook_received_at, created_at, updated_at
	...
	err := r.db.QueryRow(ctx, query, key).Scan(
		...
		&p.IdempotencyKey, &p.FailureReason, &p.WebhookReceivedAt, &p.CreatedAt, &p.UpdatedAt)
...
func (r *PaymentRepository) Update(ctx context.Context, tx pgx.Tx, p *model.Payment) error {
	...
	_, err := tx.Exec(ctx, query, p.ID, p.Status, p.GatewayPaymentID, p.FailureReason)
	return err
}

func (r *PaymentRepository) UpdateStatus(ctx context.Context, id string, status model.PaymentStatus, failureReason string) error {
